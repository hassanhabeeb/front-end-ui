...
import { deleteScrollingHighlights } from "../../../api/services/home";

const ScrollingHighlightsTable = ({
  data,
  setPage,
  limit,
  setLimit,
  ...
  const handleDelete = () => {
    ...
    if(instance_ids){
        deleteScrollingHighlights({ id: instance_ids })((response) => {
            if (response?.status && (response?.status_code == 200 || response?.status_code == 201)) {
                toast.success(response?.message || "Scrolling Highlight deleted successfully");
                setUpdate(!update);
            } else {
      ...
      cell: (row) => (
        <div className="action-wrapper">
          <Tooltip
            title="Edit"
            disableInteractive
            onClick={() => navigate(`/home/create-scrolling-highlights`, { state: { id: row?.id } })}
          >
...
export default ScrollingHighlightsTable;
