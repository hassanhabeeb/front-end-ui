...
import { deleteAboutUsBanner } from "../../../api/services/aboutUs";
...
const BannerTable = ({
  ...
  const handleDelete = () => {
    ...
    if(instance_ids){
        deleteAboutUsBanner({ id: instance_ids })((response) => {
  ...
  const columns = [
    {
     ...
     // Description
     {
      ...
      cell: (row) => {
        // Simple HTML strip
        const stripped = row.description?.replace(/<[^>]+>/g, '') || "";
        return (
            <div
            className="ellipse_email"
            data-bs-toggle="tooltip"
      ...
      cell: (row) => (
        <div className="action-wrapper">
          <Tooltip
            title="Edit"
            disableInteractive
            onClick={() => navigate(`/about-us/create-banner`, { state: { id: row.id } })}
          >
