...
import { deleteFactsAndFigures, factsAndFiguresStatusChange } from "../../../api/services/aboutUs";

const FactsAndFiguresTable = ({ data, handleSerialNumber, update, setUpdate }) => {
  ...
  const handleConfirm = () => {
    ...
    if (type === 'status') {
      factsAndFiguresStatusChange({ id: instance_ids })((res) => {
      ...
      deleteFactsAndFigures(payload)((res) => {
        if (res?.status && (res?.status_code == 200 || res?.status_code == 201)) {
      ...
      cell: (row) => (
        <div className="action-wrapper">
          <Tooltip
            title="Edit"
            disableInteractive
            onClick={() => navigate(`/about-us/create-facts-and-figures`, { state: { id: row.id } })}
          >
...
export default FactsAndFiguresTable;
